...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/MaltexadeTvDiscussion1>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/MaltexadeTvDiscussion1")]]))
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer("🚫 𝗡𝗼 𝗙𝗶𝗹𝗲 𝗪as 𝗙𝗼𝘂𝗻𝗱 🚫", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/MaltexadeTvDiscussion1")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @kingvj01
        ...
        await query.message.edit_text(
            ...
            InlineKeyboardButton("Tamil", callback_data="tamil_inf"),
            InlineKeyboardButton("English", callback_data="english_inf"),
            InlineKeyboardButton("Hindi", callback_data="hindi_inf")
        ], [
            InlineKeyboardButton("Malayalam", callback_data="malayalam_inf"),
            InlineKeyboardButton("Urdu", callback_data="urdu_inf"),
            InlineKeyboardButton("Bangla", callback_data="bangladesh_inf")
        ], [
            InlineKeyboardButton("Telugu", callback_data="telugu_inf"),
            InlineKeyboardButton("Kannada", callback_data="kannada_inf"),
            InlineKeyboardButton("Gujarati", callback_data="gujarati_inf"),
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start")
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Maltexade")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
